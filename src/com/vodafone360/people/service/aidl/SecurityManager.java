...
public class SecurityManager {
    ...
    /** Need Binder to check the originator of incoming calls */
    ...
    private SecurityManager(Context applicationContext, Binder binder) {
    ...
    protected boolean checkAuthorised() {
        try {
             ...
             */
            for (Signature sig : 
                mPackageManager.getPackageInfo(mPackageManager.getNameForUid(mBinder.getCallingUid()),
                        PackageManager.GET_SIGNATURES).signatures) {
    ...
    getInstance(Context applicationContext, Binder binderToManage){
        if (sInstance == null) {
            sInstance = new SecurityManager(applicationContext, binderToManage);
        }
